"""pcd_page.py — the HTML/CSS/JS for the dashboard UI.

A single PAGE constant rendered via render_template_string. Nothing in this
file imports any other pcd_* module — the page is pure data. The /api/*
endpoints it talks to live in pcd_api.py and pcd_chat.py.
"""
...
<style>
  ...
  /* "Active: X" line under each control — promoted to a bold colored badge
     so the user can read the current state at a glance, not as a small caption. */
  ...
  .current.dim b { background: rgba(31,111,235,0.18); color: #79c0ff; }

  /* Slider in a locked card */
  .slider-row.locked input[type=range] { opacity: 0.4; pointer-events: none; }
  .slider-row.locked .lbl { color: #7d8590; }
  ...
  .chat-fab { position: fixed; bottom: 22px; right: 22px;
              ...
              z-index: 95; transition: transform 0.15s, background 0.15s; }
  .chat-fab:hover { background: #388bfd; transform: scale(1.05); }
  .chat-panel { position: fixed; bottom: 96px; right: 22px;
                ...
                flex-direction: column; z-index: 95;
  ...
  .chat-msg.error { background: #4d1f00; color: #ffd29a;
                    border: 1px solid #db6d28; align-self: stretch; max-width: 100%; }
  .chat-thinking { color: #7d8590; font-style: italic; font-size: 0.85em;
  ...
  .chat-input textarea:focus { outline: none; border-color: #1f6feb; }
...
<div id="admin-banner" class="banner" style="display:none">
  ...
  Relaunch from an elevated PowerShell to use them.
</div>
...
<div class="grid controls">
  <div class="card"><h2>Quick Presets</h2>
    ...
    <div class="note">Combines cooling policy, turbo, CPU cap, GPU power, and Win11 overlay.</div>
  </div>

  <div class="card"><h2>Power Plan <small>powercfg</small></h2>
    <div class="btn-group" id="powerplans"></div>
    <div class="current" id="powerplans-current">—</div>
  </div>
  ...
  <div class="card"><h2>Power Mode <small>Win11 overlay</small></h2>
    ...
    <div class="current" id="overlay-current">—</div>
  </div>
  ...
  <div class="card"><h2>System Cooling Policy</h2>
    ...
    <div class="current" id="cooling-current">—</div>
    <div class="note">Passive throttles CPU before spinning fans — biggest noise reducer on AC.</div>
  </div>

  <div class="card"><h2>Turbo Boost <small>processor boost mode</small></h2>
    ...
    <div class="current" id="turbo-current">—</div>
  </div>
  ...
  <div class="card"><h2>CPU Max State</h2>
    ...
    <div class="note">99% disables turbo. 80% caps power, drops fans noticeably.</div>
  </div>
  ...
  <div class="card"><h2>GPU Power Limit <small>nvidia-smi</small></h2>
    <div class="slider-row">
...
<div class="grid metrics">
  <div class="card"><h2>CPU / Skin <small>thermal zones</small></h2><div id="zones"></div></div>
  <div class="card"><h2>NVIDIA GPU <small>nvidia-smi</small></h2><div id="nvidia"></div></div>
  <div class="card"><h2>Storage <small>NVMe SMART</small></h2><div id="storage"></div></div>
  <div class="card"><h2>Battery</h2><div id="battery"></div></div>
  <div class="card"><h2>Fan Proxy <small>CPU load &amp; turbo</small></h2><div id="cpuproxy"></div></div>
  <div class="card"><h2>OEM Sensors <small>discovered at startup</small></h2><div id="oem"></div></div>
</div>
...
<div class="chat-panel" id="chat-panel" role="dialog" aria-label="AI Assistant">
  <div class="chat-header">
    <div><b>AI Assistant</b><span class="chat-sub" id="chat-model-label">Claude</span></div>
    <div class="chat-header-btns">
      <button id="chat-reset" title="Clear conversation">⟳</button>
      <button id="chat-close" title="Close panel">×</button>
    </div>
  </div>
  <div class="chat-messages" id="chat-messages">
    <div class="chat-msg assistant">Hi! I can read your PC's live state and adjust settings.
Try: "Make it quieter", "What's running hot?", or "Show me current temps".</div>
  </div>
...
let suppressSlider = {};   // {id: timestamp} — don't overwrite sliders the user is dragging
...
// HP ZBook G11 ACPI thermal-zone code → human label.
// Unknown codes fall back to the raw name in parentheses.
const ZONE_LABELS = {
  'bagz': 'Battery Bay',
  'batz': 'Battery',
  'chgz': 'Charger / AC Adapter',
  'cpuz': 'CPU',
  'extz': 'Exhaust / Vent',
  'gfxz': 'GPU (NVIDIA)',
  'ihsz': 'CPU Heat Spreader',
  'locz': 'Mainboard',
  'pchz': 'PCH (Platform Hub)',
  'pgtz': 'CPU Package',
  'sk1z': 'Skin · Palm Rest',
  'sk2z': 'Skin · Underside',
  'sknz': 'Skin',
...
// ── Key-metrics tiles (always-visible at the top) ──
...
function paintKeyMetrics(d) {
  ...
  const skinZone = zones.find(z => /sk[12]?z/i.test(z.name));
  ...
  tiles.push(metricTile(
    ...
    cp.perf_pct != null
      ? cp.perf_pct.toFixed(0) + '% perf' + (cp.perf_pct > 105 ? ' · turbo' : '')
      : '',
    '',
  ));
  ...
  if (skinZone) {
    tiles.push(metricTile(
      'Skin temp',
      skinZone.value.toFixed(1) + ' °C',
      skinZone.name.match(/\(([^)]+)\)/) ? skinZone.name.match(/\(([^)]+)\)/)[1] : '',
      tempTileClass(skinZone.value),
    ));
  }

  if (b.charge_pct != null) {
    tiles.push(metricTile(
      'Battery',
      b.charge_pct.toFixed(0) + ' %',
      b.status || '',
      b.charge_pct < 20 ? 'crit' : (b.charge_pct < 50 ? 'warm' : 'good'),
    ));
  }

  // Profile summary — which Quick Preset, if any, matches the current state
  const preset = detectActivePreset(c);
  const overlay = OVERLAY_LABELS[c.overlay] || c.overlay || '—';
  tiles.push(metricTile(
    'Profile',
...
// Match current control values against the three Quick Presets — returns
// 'quiet' / 'balanced' / 'performance' if every relevant control is at the
// preset's value, otherwise null. Lets the dashboard highlight the matching
// preset button so the user can see at a glance which profile is live.
function detectActivePreset(c) {
  if (!c) return null;
  const cp = c.cooling_policy, tb = c.turbo_boost,
        mx = c.cpu_max_pct, mn = c.cpu_min_pct, ov = c.overlay;
  if (cp === 0 && tb === 0 && mx === 80 && ov === 'best-efficiency')
    return 'quiet';
  if (cp === 1 && tb === 2 && mx === 100 && mn === 100 && ov === 'best-performance')
    return 'performance';
  if (cp === 1 && tb === 2 && mx === 100 && ov === 'balanced')
    return 'balanced';
  return null;
}
...
async function post(url, body) {
  try {
    ...
    if (!r.ok || !d.ok) {
      flash(d.error || (d.errors && d.errors.join('; ')) || 'failed', true);
    } else {
      flash('applied');
    }
    // Server already refreshed _state["controls"] before responding, so a
    // refresh() right away pulls the new state. A second one catches slow
    // settings (Win11 overlay propagates lazily for ~500ms).
    refresh();
...
// Optimistic: highlight the clicked button immediately. The next refresh()
// either confirms it or overwrites if the server reports a different state.
...
document.getElementById('presets').addEventListener('click', e => {
  ...
  if (!b) return;
  // Presets are composite — no persistent active state on the server, so
  // highlight the clicked preset briefly to confirm the click registered.
...
function bindSlider(id, url, label, suffix) {
  const el = document.getElementById(id);
...
bindSlider('cpumax', '/api/control/cpu-max',   'CPU Max',   '%');
bindSlider('cpumin', '/api/control/cpu-min',   'CPU Min',   '%');
bindSlider('gpupl',  '/api/control/gpu-power', 'GPU Power', 'W');
...
function syncSlider(id, value) {
  // Don't fight the user — leave the slider alone for 2s after they touch it
...
function paintSystem(sys) {
  ...
  const fields = [
    ...
    ['Model',        sys.Model || sys.SystemFamily],
    ['CPU',          sys.cpu_Name],
    ['Cores',        sys.cpu_NumberOfCores],
  ...
  bar.innerHTML = fields.map(([k, v]) =>
    `<div><span class="si-key">${esc(k)}:</span><span class="si-val">${esc(v)}</span></div>`
  ).join('');
...
async function refresh() {
  try {
    ...
    // Highlight the Quick Preset whose state matches the current controls.
    ...
    if (planHolder.dataset.signature !== plans.map(p => p.guid).join(',')) {
      planHolder.innerHTML = plans.map(p =>
        `<button class="ctrl" data-guid="${esc(p.guid)}">${esc(p.name)}</button>`
      ).join('');
    ...
    // GPU slider — disable when nvidia-smi reports the GPU's power limit is
    // locked (min == max), e.g. on most laptop NVIDIA parts.
    const gpu = c.gpu;
    ...
    if (gpu) {
      if (gpu.limit_locked) {
        ...
        gpunote.textContent =
          `Locked at ${gpu.limit_w} W by the NVIDIA driver — this laptop's ` +
          `GPU does not expose user-adjustable power limits.`;
      } else {
        ...
        gpunote.textContent =
          `range ${gpu.min_w}–${gpu.max_w} W, default ${gpu.default_w} W, ` +
          `drawing ${gpu.draw_w != null ? gpu.draw_w.toFixed(1) : '—'} W`;
      }
    } else {
      gpupl.disabled = true;
      gpuRow.classList.add('locked');
    ...
    document.getElementById('nvidia').innerHTML = gpus.length
      ? gpus.map(g =>
          ...
          row('Memory',   fmt(g.clock_mem_mhz, 0, 'MHz'), g.clock_mem_mhz, 8000, '#3fb950')
        ).join('')
    ...
    let bhtml = '';
    if (b.charge_pct != null)
      bhtml += row('Charge', fmt(b.charge_pct, 0, '%'), b.charge_pct, 100, '#2ea043');
    if (b.temp_c != null)
      bhtml += row('Temp (firmware)', fmt(b.temp_c, 1, '°C'), b.temp_c, 100, tempColor(b.temp_c));
    const batZone = zones.find(z => /batz/i.test(z.name));
    if (batZone)
      bhtml += row('Temp (ACPI batz)', batZone.value.toFixed(1) + ' °C',
                   batZone.value, 100, tempColor(batZone.value));
    if (b.status)
      bhtml += `<div class="row"><div class="name">Status</div>` +
               `<div class="value" style="flex:1;text-align:left">${esc(b.status)}</div></div>`;
    if (b.runtime_min != null)
      bhtml += row('Runtime', fmt(b.runtime_min, 0, 'min'), b.runtime_min, 600, '#8b949e');
    document.getElementById('battery').innerHTML = bhtml || '<div class="empty">no battery info</div>';

    const cp = s.cpu_proxy || {};
    let chtml = '';
    if (cp.util_pct != null)
      chtml += row('CPU Util',     fmt(cp.util_pct, 0, '%'),   cp.util_pct, 100,  '#58a6ff');
    if (cp.perf_pct != null)
      chtml += row('Perf (turbo)', fmt(cp.perf_pct, 0, '%'),   cp.perf_pct, 200,  '#db6d28');
    if (cp.freq_mhz != null)
      chtml += row('Frequency',    fmt(cp.freq_mhz, 0, 'MHz'), cp.freq_mhz, 5500, '#bc8cff');
    const cpuZone = zones.find(z => /cpuz/i.test(z.name));
    if (cpuZone)
      chtml += row('CPU Zone', cpuZone.value.toFixed(1) + ' °C',
                   cpuZone.value, 100, tempColor(cpuZone.value));
    ...
    document.getElementById('oem').innerHTML = oem.length
      ? oem.map(h => {
          const kv = h.records.map(rec =>
            Object.entries(rec)
              .map(([k,v]) => `<div class="oem-kv">${esc(k)}: ${esc(v)}</div>`).join('')
          ).join('<hr style="border:0;border-top:1px solid #21262d;margin:6px 0">');
          return `<div class="oem-block">` +
                 `<div class="oem-class">${esc(h.class)}</div>` +
                 `<div class="oem-ns">${esc(h.namespace)}</div>` +
                 kv + `</div>`;
        }).join('')
      : '<div class="empty">no vendor-specific fan/thermal classes exposed on this model</div>';
...
(function initChat() {
  ...
  function appendToolCall(name, inp) {
    const args = (inp && Object.keys(inp).length)
      ? JSON.stringify(inp).slice(0, 80)
      : '';
    appendMsg('tool', '→ ' + name + (args ? ' ' + args : ''));
  }

  // Probe status on load — show clearer message if SDK or key missing.
  fetch('/api/chat/status').then(r => r.json()).then(s => {
    if (modelLbl && s.model) modelLbl.textContent = '· ' + s.model;
    if (!s.sdk_installed) {
      appendMsg('error', 'Anthropic SDK not installed. Run:  pip install anthropic\nThen restart the dashboard.');
    } else if (!s.api_key_set) {
      appendMsg('error', 'ANTHROPIC_API_KEY is not set. Get a key from https://console.anthropic.com, set it in your environment, then restart the dashboard.');
    }
  }).catch(() => {});
  ...
  resetBtn.addEventListener('click', async () => {
    ...
    log.innerHTML = '';
    appendMsg('assistant', 'Conversation cleared. Ask me anything.');
  });
  ...
  async function send() {
    ...
    sendBtn.textContent = '...';
    const thinking = appendMsg('chat-thinking', 'Thinking…');
    ...
    try {
      ...
      if (!r.ok) {
        appendMsg('error', d.error || 'request failed');
      } else {
        (d.tool_calls || []).forEach(tc => appendToolCall(tc.name, tc.input));
        appendMsg('assistant', d.reply || '(no reply)');
        // Refresh the dashboard right away — any control changes Claude made
        // will appear in the next /api/thermal response.
        refresh();
